...
public class ShooterSubsystem extends SubsystemBase {
    ...
    // Saved preset targets — restored when intake mode exits
    private double savedFlywheelMotorRPM = CLOSE_SHOT_RPM;
    private double savedHoodPoseRot      = CLOSE_SHOT_HOOD;
    ...
    /**
     * Transitions to a new state. If already in that state, no-op.
     * Use forceState() to re-command motors without a state change.
     */
    ...
    /**
     * Forces a state transition even if already in that state.
     * Use this when targets have changed and motors need to be re-commanded.
     */
    private void forceState(ShooterState newState) {
        applyState(newState);
    }

    private void applyState(ShooterState newState) {
     ...
     *
     * AlignToHubCommand calls this in initialize(), then calls prepareToShoot() to
     ...
     * AlignToHubCommand.initialize() calls setHubShotPreset() + prepareToShoot()
    ...
    /**
     * While held: saves current preset targets, sets hood to INTAKE_HOOD and
     * flywheels to IDLE_RPM so the indexer can run during intake.
     * On release: restores saved targets and returns to SPINUP so the driver
     * can re-select their shot mode.
     */
    public Command intakeModeCommand() {
        return Commands.startEnd(
            () -> {
                savedFlywheelMotorRPM  = targetFlywheelMotorRPM;
                savedHoodPoseRot       = targetHoodPoseRot;
                targetFlywheelMotorRPM = IDLE_RPM;
                targetHoodPoseRot      = INTAKE_HOOD;
                forceState(ShooterState.READY);
            },
            () -> {
                targetFlywheelMotorRPM = savedFlywheelMotorRPM;
                targetHoodPoseRot      = savedHoodPoseRot;
                setState(ShooterState.SPINUP);
            },
            this);
    }
