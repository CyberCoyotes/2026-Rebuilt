...
public class RobotContainer {
     ...
     * then increase to actual desired values once we have tested the robot's
     * responsiveness and handling at lower speeds.
    ...
    private double MaxSpeed = 0.5 * TunerConstants.kSpeedAt12Volts.in(MetersPerSecond); // kSpeedAt12Volts desired top
                                                                                        // speed
    private double MaxAngularRate = RotationsPerSecond.of(0.5).in(RadiansPerSecond); // 3/4 0.75 of a rotation per
...
// private final ClimberSubsystem climber;
    // private final IntakeCommands intakeCommands; // Not needed since we can just
    // use intake subsystem methods directly in bindings
   // private final ShooterCommands shooterCommands;
    private final ShooterCommands shooterCommands = null;
    /* Path follower */
    ...
    private void configureBindings() {
        ...
        // Default: Field-centric drive with left stick (translate) and right stick
        // (rotate)
        ...
        // Right Trigger: Flywheel ramp-up test — hold to ramp to target RPM, release to
        // idle.
         ...
         * driver.rightTrigger(0.5).whileTrue(
         * Commands.parallel(
         * ShooterCommands.rampUpFlywheel(shooter,
         * ShooterSubsystem.RAMP_TEST_TARGET_RPM),
         * Commands.waitUntil(shooter::isReady).withTimeout(5.0), // Timeout to prevent
         * indefinite waiting if something goes wrong
         * Commands.run(indexer::indexerForward).withTimeout(10.0) // Run indexer
         * forward for 10 seconds or until interrupted (e.g., by releasing trigger)
         * )
         * );
         */

        // Y: Close shot (prepare and wait for ready)
        // driver.a().onTrue(ShooterCommands.closeShot(shooter)); // TODO Comment out
        // for testing without flywheel
        // Close shot hood preset — sets target AND transitions to READY so motor
        // actually moves
        ...
        // operator.rightBumper().onTrue(shooter.runHoodToMax()); // Move hood to
        // MAX_HOOD_POSE
        // operator.leftBumper().onTrue(shooter.runHoodToMin()); // Move hood to
        // MIN_HOOD_POSE
        ...
        // operator.povUp().onTrue(ShooterCommands.increaseTargetVelocity(shooter,
        // ShooterSubsystem.FLYWHEEL_TEST_INCREMENT_RPM));
        // POV Down: Decrease flywheel velocity by 100 RPM
        // operator.povDown().onTrue(ShooterCommands.decreaseTargetVelocity(shooter,
        // ShooterSubsystem.FLYWHEEL_TEST_INCREMENT_RPM));
